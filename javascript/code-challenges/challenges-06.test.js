...
const getNames = (arr) => {
  let tempArr = arr.map((v) => {
    let tempName = [...v.name];
    const completeName = tempName.reduce((previousValue, currentValue) => currentValue + previousValue, '');
    return completeName;
  });
  return tempArr;
};
...
const addBirthYearProperty = (obj, year) => {
  Object.assign(obj, {fullName: obj.fullName, yearBorn: year});
};
