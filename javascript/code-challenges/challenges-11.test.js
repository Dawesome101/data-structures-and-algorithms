...
function transformToLis(obj){
  ...
  let tempArr2 = Object.values(obj);

  return tempArr.map((v, i) =>{
    return `<li>${v}: ${tempArr2[i]}</li>`;
  });
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach((v) => {
    ...
    slaveArr.push(tempArr);
  });

  slaveArr.forEach((v) => {
    masterArr.push(v.map(num => Math.pow(2, num)));
  });
...
let findMaleAndFemale = (data) => {
  ...
  for(let i = 0; i < data.length; i++){
    if(data[i].gender === 'male' || data[i].gender === 'female'){
      if(data.length - 1 > i ){
        masterString += data[i].name + ' and ';
      } else {
        masterString += data[i].name;
      }
    }
  }
