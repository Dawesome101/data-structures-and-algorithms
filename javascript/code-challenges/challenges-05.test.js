...
const addPurchases = (arr) => {
  const iV = 0;
  return arr.reduce((previous, current) => previous + current.purchasePrice, iV);
};
...
const countNumberOfElements = (arr) => {
  const iV = 0;
  return arr.reduce((previous) => previous + 1, iV);
};
...
const returnNames = (arr) => {
  const iV = 0;
  let names = [];
  arr.reduce((previous, current) => names.push(current.name), iV);
...
const reversedString = (str) => {
  let myStringArray = [...str];
  return myStringArray.reduce((iV, previous, current) => iV = myStringArray[current] + iV, '');
};
