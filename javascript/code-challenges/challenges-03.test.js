...
const typeNum = (arr) => {
  let result = arr.filter((v) => parseInt(v));
  return result;
};
...
const containsAnd = (arr) => {
  let result = arr.filter((v) => v.includes('and'));
  return result;
};
...
const getStatName = (arr, minBaseStat) => {
  let result = getBaseStatGreaterThan(arr, minBaseStat).name;
